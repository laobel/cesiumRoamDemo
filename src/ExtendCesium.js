...
var SimplyRoam = (function() {
    ...
    _.TrackTarget=(function() {
        function _(simplyRoam,options,positions) {
            ...
            this.path =[new Cesium.Cartesian3(this._options.icon.position.x,this._options.icon.position.y,this._options.icon.position.z)];
        ...
        _.prototype._andPoint=function (position,value) {
            this.points.push(this._simplyRoam._viewer.entities.add({
                name : value,
                position : position,
                billboard : {
        ...
        _.prototype._initIcon = function() {
            ...
            if (!Cesium.defined(this._options.icon.orientation)) {
                var heading = Cesium.Math.toRadians(60);
                var pitch = 0;
                var roll = 0;
                var hpr = new Cesium.HeadingPitchRoll(heading, pitch, roll);

                this._options.icon.orientation = Cesium.Transforms.headingPitchRollQuaternion(this.path[this.path.length - 1], hpr);
            }

            this.icon = this._simplyRoam._viewer.entities.add(this._options.icon);

            this._simplyRoam._viewer.zoomTo(this.icon);
        };
        ...
        _.prototype.addPositonsWithTime=function(positions,callback) {
            ...
            for(var i=0;i<positions.length;i++){
                this.path.push(Cesium.Cartesian3.fromDegrees(positions[i].x,positions[i].y,positions[i].z));
                postionArr.push(new Cesium.Cartesian3.fromDegrees(positions[i].x,positions[i].y,positions[i].z));
                timeArr.push(new Cesium.JulianDate.fromDate(this._initDate(positions[i].time)));

                this._andPoint(Cesium.Cartesian3.fromDegrees(positions[i].x,positions[i].y,positions[i].z),positions[i].label);
            }
